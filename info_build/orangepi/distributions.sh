...
apt -y install xorg xfce4 xfce4-goodies vlc network-manager-gnome
...
apt $_auto install xinit xserver-xorg 
apt $_auto install lxde lightdm lightdm-gtk-greeter policykit-1 --no-install-recommends
apt $_auto install net-tools
apt $_auto install lxsession-logout
apt clean

if [ "${_DST}" = "Ubuntu" ] ; then
	apt $_auto install humanity-icon-theme --no-install-recommends 
fi

apt $_auto install pulseaudio pulseaudio-utils alsa-oss alsa-utils alsa-tools libasound2-data pavucontrol
apt $_auto install smplayer 
apt $_auto install synaptic software-properties-gtk lxtask galculator policykit-1-gnome --no-install-recommends
apt clean
...
if [ "${_DST}" = "Ubuntu" ]; then
	apt $_auto install chromium-browser gvfs-fuse gvfs-backends --no-install-recommends
else
	apt $_auto install chromium gvfs-fuse gvfs-backends --no-install-recommends
fi
apt $_auto install network-manager-gnome
apt clean
...
#	cp /usr/bin/qemu-arm-static "$DEST/usr/bin"
    ...
    mount --bind /dev $DEST/dev || true
    mount --bind /dev/pts $DEST/dev/pts || true
    #mount --bind /run $DEST/run || true
    ...
    umount -f $DEST/dev/pts || true
    umount -f $DEST/dev || true
    #umount -f $DEST/run || true
...
add_opi_config_libs() {
	do_chroot apt install -y dialog expect bc cpufrequtils figlet toilet lsb-release
	...
	case $DISTRO in
		...
		bionic)
		        case $SOURCES in
		                "CDN"|"OFCL")
		                        SOURCES="http://ports.ubuntu.com"
					ROOTFS="http://cdimage.ubuntu.com/ubuntu-base/releases/18.04/release/ubuntu-base-18.04.5-base-arm64.tar.gz"
		                        ;;
		                "CN")
		                        #SOURCES="http://mirrors.aliyun.com/ubuntu-ports"
		                        SOURCES="http://mirrors.tuna.tsinghua.edu.cn/ubuntu-ports"
...
apt install -y dosfstools curl xz-utils iw rfkill ifupdown wpasupplicant openssh-server alsa-utils
apt install -y rsync u-boot-tools vim
apt install -y parted network-manager git autoconf gcc libtool
apt install -y libsysfs-dev pkg-config libdrm-dev xutils-dev hostapd
apt install -y dnsmasq apt-transport-https man subversion
apt install -y imagemagick libv4l-dev cmake bluez
timeout 560 apt install -y $EXTRADEBS

apt install -f

apt -y remove --purge ureadahead
$ADDPPACMD
apt -y update
$DISPTOOLCMD
