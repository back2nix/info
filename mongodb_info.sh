...
db.users.find({"timelastcalculateprogress": {$gt: new Date('2022-06-12')}}, {_id:1})
...
# Посчитать количество записей у которых массив photo размером больше 50
db.users.find({_id: {$lte: NumberLong(1000)}, "$expr": {$gte: [{$size: "$photo"}, 50]}}, {_id: 1}).count()

```
db.drawday.find({"userinfo.mail": { $all : ["mail@mail.ru"] }})
db.drawday.find({"userinfo.mail": "mail@mail.ru"})
```

# Update an Array Element
```
db.drawday.updateMany({"userinfo.mail": "mail@mail.ru"}, {"$set" : { "userinfo.$.mail": "mail2@mail.ru"}})
```
