...
### Вопрос №15: Что такое индексы index? Зачем мы их используем?
...
* **B-Tree (сбалансированное дерево)** — самый распространенный тип, эффективен для диапазонов, сортировки и точного поиска.
* **Hash (хеш-индекс)** — используется только для поиска точного совпадения (=), работает быстрее B-Tree, но не поддерживает диапазоны.
* **Bitmap (битовый)** — идеален для столбцов с малым количеством уникальных значений (низкая кардинальность), например, пол или статус.
* **Кластерный (Clustered)** — физически упорядочивает строки таблицы на диске; может быть только один на таблицу.
* **Некластерный (Non-clustered)** — хранится отдельно от данных и содержит ссылки (указатели) на реальные строки таблицы.
* **Уникальный (Unique)** — помимо ускорения поиска, гарантирует отсутствие дубликатов в столбце.
* **Составной (Composite)** — индекс, построенный сразу по нескольким столбцам для оптимизации сложных запросов.
* **Полнотекстовый (Full-text)** — предназначен для поиска ключевых слов внутри больших текстовых полей.
