...
a='REPO_DEPRECATED\/';b='hello\/' && grep -l "$a" `find  -type f \( -name "*.mod" -o -name "*.go" \)` | xargs sed -i "s/$a/$b/g"
...
sudo vim /etc/fstab
#UUID=cbcc1da9-8f81-404f-8d40-d7bf4af1292b       /mnt/mongo_db_base       ext4   auto,nofail,nobootwait,user,rw    0 0
#UUID=e3c3ce6c-3b39-4701-bce7-85772c2e133f       /rawdata       ext4   auto,nofail,nobootwait,,user,rw    0 0
#e3c3ce6c-3b39-4701-bce7-85772c2e133f
#UUID=0736dc01-3146-4e10-a9b3-986f9949c00d-3f60-4331-8d85-f00c94092965       /rawdata1       ext4   auto,nofail,nobootwait,user,rw    0 0
...
journalctl -f -u camera.service --output cat
...
sudo journalctl -f -u opayterminal --since "2021-09-08 08:00" --until "2021-09-08 10:00"
...
```
find /usr/share -type f -perm /a+x -exec ldd {} \; \
| grep so \
| sed -e '/^[^\t]/ d' \
| sed -e 's/\t//' \
| sed -e 's/.*=..//' \
